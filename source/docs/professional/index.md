...
[^2]: ~~It seems that without the `#commitnumber` (e.g: `github:badlydrawnrob/print-first-css` in dev dependencies) running `npm update --save` will automatically update all packages, including the very latest `print-first-css` Github repo commit.~~ Otherwise running `npm update <github-repo-package>` [won't update](https://github.com/badlydrawnrob/anki/issues/65) properly. I keep all the packages up-to-date anyway, but if you have any issues you might have to change the `#commit` number in `package.json` and run `npm install print-first-css` to force it to update. [`npm audit`](https://docs.npmjs.com/cli/v10/commands/npm-audit) and `npm audit fix` can remove other errors (follow the prompts).
